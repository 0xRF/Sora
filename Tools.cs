...
namespace Sora
{
    ...
    {
        public static async Task<IUser> GetUser(ISocketMessageChannel channel, string username)
        {
            var users = new List<IUser>();
            users.AddRange(await channel.GetUsersAsync(CacheMode.AllowDownload).Flatten());

            foreach(var user in users)
            {
                if (FuzzyMatch(user.Username, username))
                    return user;
            }
